...
- **Your broker**, only if you connected one, with the keys you gave it, read-only. The fifteen shipped adapters (Alpaca, Angel One, Charles Schwab, Dhan, Groww, ICICI Direct, Interactive Brokers, Longbridge, Questrade, Saxo Bank, tastytrade, Tradier, Trading 212, Upstox, Zerodha) use each broker's documented read endpoints, and eleven of them also read live prices through the broker's documented market data call. None of them places an order. The brokers that name a stock by its ISIN or by the exchange's number get it from Upstox's public instrument list (assets.upstox.com), fetched once a day with no login and nothing about you in the request. Where a broker signs in with a code from an authenticator app, the secret behind that app sits in `.env` with the broker's other keys and the six digits are worked out on this computer; the secret goes nowhere.
