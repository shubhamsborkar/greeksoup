...
- **Paste Token** is only for a broker that asks for a daily login (ICICI Direct today), explained under *Every morning*; the Settings screen does the same job inside the page for every such broker.
...
- **Desk · Home** connects to a broker account in whatever market you trade: holdings priced live, cash, and where the broker reports them, open futures and options, margin used and an options tape on the index. You pick the broker on Settings. Six connect as the desk comes, read-only, each with what it hands out from its own dashboard: Alpaca, ICICI Direct, Interactive Brokers, Tradier, Trading 212 and Zerodha. The currency and the market hours follow the broker. ICICI Direct is the one that also serves live ticks, futures and options, margin and the options tape, because the desk was built against it; the others return holdings and cash and the desk prices them. Any other broker with an API is one file your agent writes from that broker's documentation, to the shape described in the brokers folder, and Settings lists the brokers the desk knows about without a file and the way in for each. A broker with no API exports a file, and Desk · Book takes it.

So a reader in the US picks Alpaca, Interactive Brokers or Tradier and has both desks; a reader in the UK picks Trading 212 or Interactive Brokers; a reader in India picks ICICI Direct or Zerodha; a reader in Australia has the agent write the file for an ASX broker, or keeps the book by hand while it is written.
