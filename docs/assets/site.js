...
(function () {
  ...
  function setTheme(t, save) {
    ...
    d.querySelectorAll("[data-theme-set]").forEach(function (b) { b.classList.toggle("on", b.dataset.themeSet === t); });
    if (save) { try { localStorage.setItem("gs-theme", t); } catch (e) {} }
  }
  d.querySelectorAll("[data-theme-set]").forEach(function (b) { b.addEventListener("click", function () { setTheme(b.dataset.themeSet, true); }); });
  ...
  /* operating system, shared by every install block */
  function setOS(os) {
  ...
  /* copy: the visible line in the nearest install block */
  d.querySelectorAll("[data-copy-for]").forEach(function (b) {
    b.addEventListener("click", function () {
      var box = b.closest(".card") || d;
      var code = Array.prototype.find.call(box.querySelectorAll("[data-install] code"), function (c) { return !c.hidden; });
      var text = code ? code.textContent : "";
      var label = b.innerHTML;
      function done() { b.innerHTML = label.replace(/Copy the line/, "Copied"); setTimeout(function () { b.innerHTML = label; }, 1600); }
  ...
  idle(function () { tabs.forEach(function (t) { preload(t.dataset.shot); }); });
  ...
  fetch(URL, { cache: "no-store" }).then(function (r) { if (!r.ok) throw new Error(r.status); return r.text(); }).then(function (text) {
    ...
    if (rel) rel.innerHTML = rows.slice(0, 6).map(function (r, i) {
